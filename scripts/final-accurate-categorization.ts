...
// Accurate categorization based on actual business name analysis
function categorizeBusinessAccurately(businessName: string, email: string): string {
  const name = businessName.toLowerCase();
  const emailLower = email.toLowerCase();
  
  // First, check for numbered companies (BC LTD, CANADA INC, etc.)
  ...
  // Check for personal names (First Last format, no business descriptors)
  ...
  // Check for construction (very specific keywords)
  ...
  // Check for healthcare (very specific keywords)
  ...
  // Check for automotive (very specific keywords)
  ...
  // Check for food/restaurants (very specific keywords)
  ...
  // Check for retail (very specific keywords)
  ...
  // Check for professional services (very specific keywords)
  ...
  // Check for technology (very specific keywords)
  ...
  // Check for financial (very specific keywords)
  ...
  // Check for real estate (very specific keywords)
  ...
  // Check for entertainment (very specific keywords)
  ...
  // Check for manufacturing (very specific keywords)
  ...
  // Check for transportation (very specific keywords)
  ...
  // Check for forestry (very specific keywords)
  ...
  // Check for mining (very specific keywords)
...
async function finalAccurateCategorization() {
  console.log('🎯 Final Accurate Categorization...\n');
  ...
  try {
    ...
    for (const member of audienceMembers) {
      const newCategory = categorizeBusinessAccurately(member.businessName, member.primaryEmail);
    ...
    console.log('\n👥 Creating accurate audience groups...');
    ...
    for (const [category, members] of newCategorization) {
      ...
      const audienceGroup = await prisma.audienceGroup.create({
        data: {
          name: groupName,
          description: `Businesses in ${groupName.toLowerCase()} industry - accurate categorization based on actual business name analysis`,
          criteria: {
            industry: category,
            minMembers: members.length,
            categorizationMethod: 'accurate_business_name_analysis',
            improved: true,
            semanticAnalysis: true,
            comprehensiveKeywords: true,
            accuratePatternMatching: true
          }
        }
      });
        ...
        await prisma.audienceMember.create({
          data: {
            ...
            meta: {
              ...
              categorizedBy: 'accurate_business_name_analysis'
            }
          }
        });
      }
    ...
    // Final summary
    console.log('\n🎉 Final Accurate Categorization Complete!');
    console.log('\n📊 Final Summary:');
