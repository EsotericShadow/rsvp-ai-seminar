...
// Comprehensive industry categorization including missing major industries
const COMPREHENSIVE_INDUSTRY_CATEGORIZATION = {
  ...
  'FORESTRY_LOGGING': [
    'forestry', 'logging', 'log', 'logs', 'lumber', 'lumbering', 'timber', 'timbers',
    'sawmill', 'saw mills', 'mill', 'mills', 'wood', 'wood products', 'wooden',
    'lumber yard', 'lumberyard', 'forest', 'forests', 'forest management', 'tree',
    'trees', 'tree service', 'tree services', 'arborist', 'arborists', 'cutting',
    'harvesting', 'harvest', 'harvester', 'harvesters', 'chipper', 'chippers',
    'mulch', 'mulching', 'firewood', 'fire wood', 'pulp', 'pulp mill', 'paper',
    'paper mill', 'paper products', 'fiber', 'fibre', 'biomass', 'reforestation',
    'silviculture', 'silvicultural', 'forest products', 'wood chips', 'woodchips',
    'lumber products', 'timber products', 'wood manufacturing', 'forestry services',
    'logging contractor', 'logging contractors', 'forest contractor', 'forest contractors'
  ],
  
  'MINING_RESOURCES': [
    'mining', 'mine', 'mines', 'miner', 'miners', 'minerals', 'mineral', 'ore',
    'ores', 'coal', 'coal mining', 'gold', 'gold mining', 'silver', 'silver mining',
    'copper', 'copper mining', 'iron', 'iron mining', 'steel', 'steel production',
    'metal mining', 'metals', 'metallic', 'quarry', 'quarries', 'quarrying',
    'aggregate', 'aggregates', 'gravel', 'gravel pit', 'sand', 'sand pit',
    'crushed stone', 'stone', 'rocks', 'rock', 'drilling', 'driller', 'drillers',
    'exploration', 'explore', 'explorer', 'explorers', 'prospecting', 'prospect',
    'prospector', 'prospectors', 'geology', 'geological', 'geologist', 'geologists',
    'survey', 'surveying', 'surveyor', 'surveyors', 'extraction', 'extract',
    'excavation', 'excavate', 'excavator', 'excavators', 'bulldozer', 'bulldozers',
    'heavy equipment', 'equipment', 'machinery', 'machines', 'trucking', 'hauling',
    'haul', 'haulage', 'transport', 'transportation', 'resource', 'resources',
    'natural resources', 'energy', 'oil', 'gas', 'petroleum', 'natural gas',
    'drilling', 'oil drilling', 'gas drilling', 'well', 'wells', 'rig', 'rigs',
    'platform', 'platforms', 'offshore', 'onshore', 'production', 'produce',
    'refinery', 'refineries', 'processing', 'process', 'refining', 'refine'
  ],
  ...
  'HEALTHCARE_MEDICAL': [
    ...
    'counseling', 'counselor', 'counsellor', 'psychiatry', 'psychiatrist',
...
function categorizeBusinessByName(businessName: string, email: string): string {
  ...
  for (const [category, keywords] of Object.entries(COMPREHENSIVE_INDUSTRY_CATEGORIZATION)) {
  ...
  // Check for numbered companies and personal names
  const numberedPattern = /\d{7,8}\s+(bc\s+)?ltd|bc\s+\d{7,8}|canada\s+inc\.?\s*\d{7,8}/i;
  const personalNamePattern = /^[A-Z][a-z]+\s+[A-Z][a-z]+(\s+[A-Z][a-z]+)*\s*(inc|corp|ltd|llc|llp)\.?$/i;
  ...
  if (personalNamePattern.test(name) || justNamePattern.test(name)) {
    return 'PERSONAL_NAMES_NUMBERED';
  }
...
async function comprehensiveIndustryCategorization() {
  console.log('🎯 Comprehensive Industry Categorization...\n');
  ...
  try {
    ...
    console.log('\n📊 New Comprehensive Categorization:');
    ...
    console.log('\n👥 Creating comprehensive audience groups...');
    ...
    for (const [category, members] of newCategorization) {
      ...
      const audienceGroup = await prisma.audienceGroup.create({
        data: {
          name: groupName,
          description: `Businesses in ${groupName.toLowerCase()} industry - comprehensive categorization including forestry, mining, and proper personal name detection`,
          criteria: {
            industry: category,
            minMembers: members.length,
            categorizationMethod: 'comprehensive_industry_analysis',
            improved: true,
            semanticAnalysis: true,
            comprehensiveKeywords: true,
            includesForestry: true,
            includesMining: true,
            properPersonalNameDetection: true
          }
        }
      });
        ...
        await prisma.audienceMember.create({
          data: {
            ...
            meta: {
              ...
              categorizedBy: 'comprehensive_industry_analysis'
            }
          }
        });
      }
    ...
    // Final summary
    console.log('\n🎉 Comprehensive Industry Categorization Complete!');
    console.log('\n📊 Final Summary:');
    ...
    console.log('\n📋 Comprehensive Audience Groups:');
    finalGroups.forEach(group => {
      console.log(`   • ${group.name}: ${group._count.members} businesses`);
    });

  } catch (error) {
    console.error('❌ Error in comprehensive categorization:', error);
  } finally {
    await prisma.$disconnect();
  }
}

comprehensiveIndustryCategorization();
