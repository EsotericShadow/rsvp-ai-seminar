...
/**
 * Simple Weaviate Connection Test (JavaScript)
 * Tests basic Weaviate connectivity without TypeScript complications
 */
...
async function testWeaviateConnection() {
  ...
  try {
    ...
    for (const expected of expectedCollections) {
      if (collections.includes(expected)) {
        ...
        try {
          ...
          console.log(`      ⚠️  Could not query sample: ${error.message || 'Unknown error'}`);
        }
      } else {
    ...
    const testQueries = ['campaign', 'Evergreen', 'AI in Terrace', 'RSVP system'];
    ...
    for (const query of testQueries) {
      try {
        ...
        console.log(`   "${query}": ❌ Search failed - ${error.message || 'Unknown error'}`);
      }
    }
