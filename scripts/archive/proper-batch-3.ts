...
async function properBatch3() {
  console.log('🎯 PROPER BATCH 3 - CORRECTED LOGIC')
  console.log('===================================')
  ...
  try {
    ...
    for (const business of businesses) {
      ...
      let targetGroup = 'Chains & Franchises'
      let reason = 'Default to chains'
      ...
      if (targetGroup !== 'Chains & Franchises') {
        corrections.push({ business, targetGroup, reason })
      }
    }
    
    console.log(`\n📊 BATCH 3 RESULTS:`)
    console.log(`- Total businesses: ${businesses.length}`)
    console.log(`- Staying in Chains: ${businesses.length - corrections.length}`)
    console.log(`- Moving to other categories: ${corrections.length}`)
    
    if (corrections.length > 0) {
      console.log(`\n🔧 APPLYING CORRECTIONS:`)
      
      for (const correction of corrections) {
        const targetGroupId = groupMap.get(correction.targetGroup)
        if (targetGroupId) {
          await prisma.audienceMember.update({
            where: { id: correction.business.id },
            data: { 
              groupId: targetGroupId,
              unsubscribed: correction.targetGroup === 'Chains & Franchises'
            }
          })
          console.log(`   ✅ Moved "${correction.business.businessName}" to ${correction.targetGroup}`)
        }
      }
    }
