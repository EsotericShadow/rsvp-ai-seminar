...
async function createOfficialTestCampaign() {
  console.log('🚀 Creating official test campaign with RSVP tracking...')

  try {
    // 1. Create official email template
    ...
    // 2. Create official test campaign
    ...
    const officialCampaign = await prisma.campaign.create({
      data: {
        ...
        description: 'Official test campaign for AI tools session invitation with RSVP tracking',
        status: 'DRAFT',
        meta: {
          official: true,
          createdBy: 'official-test',
          ...
          respectsEmailLimit: true
        }
      }
    })

    // 3. Get audience groups
    const groups = await prisma.audienceGroup.findMany({
      where: { meta: { path: ['official'], equals: true } }
    })

    // 4. Create campaign schedules for each group (respecting 100 email limit)
    ...
    for (let i = 0; i < groups.length; i++) {
      ...
      await prisma.campaignSchedule.create({
        data: {
          ...
          status: 'SCHEDULED',
          stepOrder: i + 1,
          throttlePerMinute: 10, // Respect 100 email limit
          meta: {
            official: true,
            createdBy: 'official-test',
    ...
    // 5. Update global template settings
    ...
    // 6. Test RSVP tracking by sending one email
    ...
    // 7. Summary
    console.log('\n🎉 Official test campaign created successfully!')
    console.log('📊 Summary:')
    console.log(`- Created official email template with your exact HTML`)
    console.log(`- Created official test campaign`)
    console.log(`- Created ${groups.length} campaign schedules`)
    ...
    console.log(`- Respects 100 email per 24 hours limit`)
    
    console.log('\n🔗 Test RSVP Link:')
    console.log(`https://rsvp.evergreenwebsolutions.ca/rsvp?eid=biz_official-business-1&campaign=${officialCampaign.id}`)
    
    console.log('\n📧 Ready to test with:')
    console.log('- gabriel.lacroix94@icloud.com (resubscribed)')
    console.log('- greenalderson@gmail.com')
    console.log('- gabriel@evergreenwebsolutions.ca')
    console.log('- Tangible18@outlook.com')

  } catch (error) {
    console.error('❌ Official test campaign creation failed:', error)
    throw error
  } finally {
