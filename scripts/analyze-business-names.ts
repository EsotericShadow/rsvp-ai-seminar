...
async function analyzeBusinessNames() {
  console.log('🔍 Analyzing Business Names for Better Categorization...\n');
  ...
  try {
    ...
    console.log(`📊 Found ${audienceMembers.length} audience members to analyze\n`);
    ...
    for (const member of audienceMembers) {
      ...
      if (/\d{7,8}\s+(bc\s+)?ltd|bc\s+\d{7,8}|canada\s+inc\.?\s*\d{7,8}|\d{7,8}\s+bc\s+ltd|\d{7,8}\s+canada\s+inc/i.test(member.businessName)) {
        patterns.numberedCompanies.push(member.businessName);
      }
      // Check for personal names (First Last format)
      else if (/^[A-Z][a-z]+\s+[A-Z][a-z]+(\s+[A-Z][a-z]+)*\s*(inc|corp|ltd|llc|llp)\.?$/i.test(member.businessName) || 
               /^[A-Z][a-z]+\s+[A-Z][a-z]+$/i.test(member.businessName)) {
        patterns.personalNames.push(member.businessName);
      }
      // Check for construction
      else if (name.includes('construction') || name.includes('contracting') || name.includes('contractor') || 
        ...
        patterns.construction.push(member.businessName);
      }
      // Check for healthcare
      else if (name.includes('medical') || name.includes('medicine') || name.includes('healthcare') || 
               name.includes('health') || name.includes('clinic') || name.includes('hospital') || 
               name.includes('doctor') || name.includes('physician') || name.includes('nurse') || 
               name.includes('dental') || name.includes('dentist') || name.includes('orthodontist') || 
               name.includes('denture') || name.includes('oral') || name.includes('surgery') || 
               name.includes('surgeon') || name.includes('pharmacy') || name.includes('pharmacist') || 
               name.includes('therapy') || name.includes('therapist') || name.includes('massage') || 
               name.includes('chiropractic') || name.includes('chiropractor') || name.includes('acupuncture') || 
               name.includes('naturopathic') || name.includes('naturopath') || name.includes('holistic') || 
               name.includes('wellness') || name.includes('mental health') || name.includes('psychology') || 
               name.includes('psychologist') || name.includes('counseling') || name.includes('counselor') || 
               name.includes('psychiatry') || name.includes('psychiatrist') || name.includes('optometry') || 
               name.includes('optometrist') || name.includes('eye care') || name.includes('vision') || 
               name.includes('hearing') || name.includes('audiology') || name.includes('audiologist') || 
               name.includes('hearing aids') || name.includes('lab') || name.includes('laboratory') || 
               name.includes('testing') || name.includes('diagnostic') || name.includes('diagnosis') || 
               name.includes('x-ray') || name.includes('imaging') || name.includes('ultrasound') || 
               name.includes('mri') || name.includes('ct') || name.includes('scan') || name.includes('pathology') || 
               name.includes('pathologist') || name.includes('blood') || name.includes('urine') || 
               name.includes('sample')) {
        patterns.healthcare.push(member.businessName);
      }
      // Check for automotive
      else if (name.includes('auto') || name.includes('automotive') || name.includes('car') || 
               ...
               name.includes('garage') || name.includes('shop') || name.includes('dealership') || 
               name.includes('dealer') || name.includes('sales') || name.includes('leasing') || 
               name.includes('rental') || name.includes('towing') || name.includes('tow') || 
               name.includes('recovery') || name.includes('collision') || name.includes('body shop') || 
               name.includes('bodywork') || name.includes('detailing') || name.includes('detail') || 
               name.includes('wash') || name.includes('washing') || name.includes('cleaning') || 
               name.includes('auto glass') || name.includes('windshield') || name.includes('exhaust') || 
               name.includes('muffler') || name.includes('performance') || name.includes('tuning') || 
               name.includes('custom')) {
        patterns.automotive.push(member.businessName);
      }
      // Check for food/restaurants
      else if (name.includes('restaurant') || name.includes('food') || name.includes('dining') || 
        ...
        patterns.food.push(member.businessName);
      }
      // Check for retail
        ...
        patterns.retail.push(member.businessName);
      }
      // Check for professional services
      else if (name.includes('consulting') || name.includes('consultant') || name.includes('advisory') || 
        ...
        patterns.professional.push(member.businessName);
      }
      // Check for technology
      else if (name.includes('technology') || name.includes('tech') || name.includes('information technology') || 
        ...
        patterns.technology.push(member.businessName);
      }
      // Check for financial
      else if (name.includes('financial') || name.includes('finance') || name.includes('financing') || 
        ...
        patterns.financial.push(member.businessName);
      }
      // Check for real estate
      else if (name.includes('real estate') || name.includes('realtor') || name.includes('realty') || 
        ...
        patterns.realEstate.push(member.businessName);
      }
      // Check for entertainment
      else if (name.includes('entertainment') || name.includes('entertain') || name.includes('recreation') || 
        ...
        patterns.entertainment.push(member.businessName);
      }
      // Check for manufacturing
      else if (name.includes('manufacturing') || name.includes('manufacturer') || name.includes('manufacturers') || 
        ...
        patterns.manufacturing.push(member.businessName);
      }
      // Check for transportation
      else if (name.includes('transportation') || name.includes('transport') || name.includes('transporting') || 
        ...
        patterns.transportation.push(member.businessName);
      }
      // Check for forestry
      else if (name.includes('forestry') || name.includes('logging') || name.includes('log') || 
        ...
        patterns.forestry.push(member.businessName);
      }
      // Check for mining
      else if (name.includes('mining') || name.includes('mine') || name.includes('mines') || 
        ...
        patterns.mining.push(member.businessName);
      }
      // Everything else goes to other
      else {
        patterns.other.push(member.businessName);
      }
    }

    // Display analysis results
    console.log('📊 Business Name Pattern Analysis:\n');
    
    console.log(`🔢 Numbered Companies: ${patterns.numberedCompanies.length}`);
    console.log(`👤 Personal Names: ${patterns.personalNames.length}`);
    console.log(`🏗️  Construction: ${patterns.construction.length}`);
    console.log(`🏥 Healthcare: ${patterns.healthcare.length}`);
    console.log(`🚗 Automotive: ${patterns.automotive.length}`);
    console.log(`🍕 Food/Restaurants: ${patterns.food.length}`);
    console.log(`🛍️  Retail: ${patterns.retail.length}`);
    console.log(`💼 Professional Services: ${patterns.professional.length}`);
    console.log(`💻 Technology: ${patterns.technology.length}`);
    console.log(`💰 Financial: ${patterns.financial.length}`);
    console.log(`🏠 Real Estate: ${patterns.realEstate.length}`);
    console.log(`🎭 Entertainment: ${patterns.entertainment.length}`);
    console.log(`🏭 Manufacturing: ${patterns.manufacturing.length}`);
    console.log(`🚛 Transportation: ${patterns.transportation.length}`);
    console.log(`🌲 Forestry: ${patterns.forestry.length}`);
    console.log(`⛏️  Mining: ${patterns.mining.length}`);
    console.log(`📦 Other: ${patterns.other.length}`);

    // Show some examples from each category
    console.log('\n📋 Examples from each category:\n');
    
    for (const [category, businesses] of Object.entries(patterns)) {
    ...
    console.error('❌ Error analyzing business names:', error);
  } finally {
    await prisma.$disconnect();
  }
}

analyzeBusinessNames();
